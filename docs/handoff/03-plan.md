...
> **2026-09-02 정정 — 아래 원래 권고(도메인별 분할)는 틀렸다. 실행 시 순차 분할로 변경했다.**
>
> 실제 파일을 분석해보니 **셀렉터 91개가 중복 정의**돼 있고 그중 **54개는 1000줄 이상
> 떨어져** 재정의된다. `.nav-links` 5곳, `.sidebar-nav-elite` 6곳, `:root` 조차 4곳이다.
> 즉 이 파일은 뒤 규칙이 앞 규칙을 의도적으로 덮어쓰는 **누적형 구조**다.
> 도메인별로 옮기면 번들 내 순서가 뒤바뀌어 덮어쓰기가 깨진다.
> 빌드는 통과하는데 화면만 달라지는, 가장 잡기 어려운 종류의 회귀다.
>
> **채택한 방식:** 최상위 경계에서 순서를 그대로 보존해 8개로 순차 분할하고
> (`src/styles/01-…` ~ `08-…`), `index.css`는 `@import` 순서만 정의하는 22줄 파일로 만든다.
> 파일명 앞의 번호가 곧 캐스케이드 순서다.
>
> **검증:** 분할 전후로 빌드된 CSS가 **바이트 단위로 동일**함을 확인했다
> (89,813 bytes, sha256 `c7cd07b2…`, 출력 파일명까지 `index-HMXws8I1.css` 로 동일).
> 렌더 결과가 변하지 않았다는 증명이다.

원래 권고(참고용): 이미 존재하는 축(`tour-theme.css` / `dashboard.css` / `PremiumLanding.css`)을
따라 도메인별로 나누고, `index.css`에는 전역 토큰·리셋·공용 유틸만 남긴다.
— 이 방향은 캐스케이드를 먼저 정리한 뒤에야 가능하다. 후속 과제로 남긴다.
